...
{\bf Fano's inequality:} Let $P_e=Pr\{\hat{X}(Y)\neq X\}$. Then $H(P_e)+P_e \log |\chi| \geq H(X|Y)$. Weak: $P_e \geq \frac{H(X|Y)-1}{log(|X|)}$

{\bf Inequality.} If $X$ and $X'$ are independent and identically distributed then $Pr(X=X') \geq 2^{-H(X)}$. $2^{Elog(p(X))} \leq E2^{log(p(X))}, 2^{-H(X)} \leq \sum p(x)^2$ 
