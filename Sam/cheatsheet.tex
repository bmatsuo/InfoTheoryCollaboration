...
\\\\
\textit{Chp. 4 Summary} Entropy Rate: $H(\chi) = lim_{n \to \infty} \frac{1}{n} H(X^n)$; $H^\prime(\chi) = lim_{n \to \infty} H(X_n | X^{n-1})$. If stationary and stochastic: $H(\chi) = H^\prime(\chi)$. Entropy rate of stationary Markov chain (transition matrix $P$) : $H(\chi) = H(X_2|X_1) = -\sum_{i,j} \mu_i P_{ij}log(P_ij)$. \textit{functions of Markov chain}: $X^n$ is stationary Markov chain, and $Y_i = \phi(X_i)$ then $H(Y_n | Y^{n-1}, X_1) \leq H(\gamma) \leq H(Y_n|Y^{n-1})$ and $lim_{n \to \infty}H(Y_n|Y^{n-1},X_1) = H(\gamma) = lim_{n \to \infty} H(Y_n | Y^{n-1})$. \textit{Ces\'{a}ro mean} If $a_n \to a$ and $b_n = \frac{1}{n}\sum_{i=1}^n a_i$ then $b_n \to a$. \textit{2 state Markov Chain}: given $P=[1-\alpha, \alpha; \beta, 1-\beta]$ the stationary distribution $u$ where $uP = u$ is $u_1 = \frac{\beta}{\alpha+\beta}$, $u_2 = \frac{\alpha}{\alpha+\beta}$, and $H(\chi) = \frac{\beta}{\alpha + \beta}H(\alpha) + \frac{\alpha}{\alpha+\beta}H(\beta)$.
