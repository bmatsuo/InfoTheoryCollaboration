...
\begin{eqnarray}
    ...
    \label{eq: 3.11a}
\end{eqnarray}
\textbf{(b)} Justify the steps in the chain of inequalities
\begin{eqnarray}
    1-\varepsilon-\delta &\leq& \Pr(A_\varepsilon^{(n)} \cap B_\delta^{(n)}) 
        \label{eq: 3.34} \\
    &=& \sum_{A_\varepsilon^{(n)} \cap B_\delta^{(n)}} p(x^n)
        \label{eq: 3.35} \\
    &\leq& \sum_{A_\varepsilon^{(n)} \cap B_\delta^{(n)}} 2^{-n(H-\varepsilon)}
        \label{eq: 3.36} \\
    &=& |A_\varepsilon^{(n)} \cap B_\delta^{(n)}|2^{-n(H-\varepsilon)}
        \label{eq: 3.37} \\
    &\leq& |B_\delta^{(n)}|2^{-n(H-\varepsilon)}
        \label{eq: 3.38}
\end{eqnarray}

\pref{eq: 3.34} follows from inequality \pref{eq: 3.11(a) conclusion}.
\pref{eq: 3.35} comes from the definition of the probability of an event.
\pref{eq: 3.36} is true by definition of AEP. 
For $x^n \in A_\varepsilon^{(n)}$ $p(x^n) \leq 2^{-n(H - \varepsilon)}$.
In \pref{eq: 3.37}, the sum is carried out over a constant summand 
(not dependant on the sequence $x^n$).
For \pref{eq: 3.38}, the intersection's cardinality is no greater than the cardinality of either set individually.
