...
\end{align}
\thm[Kraft's Inequality] \label{thm: kraft}
...
\end{align}
\thm[Expected Length and Entropy] \label{thm: codewordentropy}
...
\thm[Optimal Codeword Lengths] \label{thm:optimum codewords}
...
\thm[Wrong Code] \label{thm:wrongcode}
...
\lemma{Huffman Properties}\label{huffman}
\begin{enumerate}
...
L(p) = L^{*}(p)
\end{eqnarray}
\thm[Optimal Huff] \label{thm: opthuff}
...
\thm[Dyadic Stuff] \label{thm: dyadic}
